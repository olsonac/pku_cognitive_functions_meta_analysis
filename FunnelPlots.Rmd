...
FPlot<-ggplot(SelectedData,aes(x=g,y=ExpN)) +
  geom_point(pch=21,fill="grey",size=2) + 
...
```
```{r}
ggsave(paste0(ForestPlotsFolder,"/FunnelPlotPKU.tiff"), plot=FPlot, units="mm", width=220, height=100, dpi=300)
```
...
FPlotBounds<-ggplot(MyData,aes(x=g,y=ExpN)) + geom_polygon(data=ESRange,aes(x=x,y=y),color="darkgrey",alpha=0.1,linetype="dashed") +
  geom_point(pch=21,fill="grey",size=2) + 
