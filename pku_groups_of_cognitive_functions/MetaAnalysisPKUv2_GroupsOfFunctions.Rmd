...
M2<-rma.mv(g, # for estimates of coefficients
           Vg,
...
M2_no_reml<-rma.mv(g, # for model comparison below
           Vg,
           random = formula(RandomTermString), 
           mods = ~ FunctionGroup,
           test="z", 
...
Overall_ES<-M2$beta[,1]
Overall_se<-M2$se
Overall_z_value<-M2$zval
Overall_p_value<-M2$pval
```

```{r, results='asis'}
M2 %>% tidy() %>% kable() %>% kable_styling()
```
...
write.csv(tidy(M2),paste0(FunctionResultsFolder,"/",ContrastTitle,"_contrast.csv"))
write.csv(glance(M2),paste0(FunctionResultsFolder,"/",ContrastTitle,"_contrast_details.csv"))

```

```{r}
WtMatrix<-weights(M2,type="matrix")
...
MNoModsREML<-rma.mv(g, # for coefficients
           Vg,
           random = formula(RandomTermString), 
           test="z", 
           data=GroupData, 
...
MNoMods<-rma.mv(g, # for model comparison
           Vg,
           random = formula(RandomTermString), 
           test="z", 
           data=GroupData,
...
ContrastTitle<-paste0(unique(GroupData$FunctionGroup),collapse="_vs_")
write.csv(tidy(MNoModsREML),paste0(FunctionResultsFolder,"/",ContrastTitle,"_contrast_no_mods_model.csv"))
write.csv(glance(MNoModsREML),paste0(FunctionResultsFolder,"/",ContrastTitle,"_contrast_no_mods_model_details.csv"))
...
MPheOnlyREML<-rma.mv(g, # for coefficients
           Vg,
           random = formula(RandomTermString), 
           mods = ~ CentredPhe,
           test="z", 
...
MPheOnly<-rma.mv(g, # for model comparison
           Vg,
           random = formula(RandomTermString), 
           mods = ~ CentredPhe,
           test="z", 
...
ContrastTitle<-paste0(unique(GroupData$FunctionGroup),collapse="_vs_")
write.csv(tidy(MPheOnlyREML),paste0(FunctionResultsFolder,"/",ContrastTitle,"_contrast_Phe_only_model.csv"))
write.csv(glance(MPheOnlyREML),paste0(FunctionResultsFolder,"/",ContrastTitle,"_contrast_Phe_only_model_details.csv"))
...
MGroupOnlyREML<-rma.mv(g, # for coefficients
           Vg,
           random = formula(RandomTermString), 
           mods = ~ FunctionGroup,
           test="z", 
...
MGroupOnly<-rma.mv(g, # for model comparison
           Vg,
           random = formula(RandomTermString), 
           mods = ~ FunctionGroup,
           test="z", 
...
ContrastTitle<-paste0(unique(GroupData$FunctionGroup),collapse="_vs_")
write.csv(tidy(MGroupOnlyREML),paste0(FunctionResultsFolder,"/",ContrastTitle,"_contrast_group_only_model.csv"))
write.csv(glance(MGroupOnlyREML),paste0(FunctionResultsFolder,"/",ContrastTitle,"_contrast_group_only_model_details.csv"))
...
MGroupAndPheREML<-rma.mv(g,
           Vg,
           random = formula(RandomTermString), 
           mods = ~ FunctionGroup + CentredPhe,
           test="z", 
           data=GroupData, 
           method="REML"
           )

MGroupAndPhe<-rma.mv(g, # for model comparison
           Vg,
           random = formula(RandomTermString), 
           mods = ~ FunctionGroup + CentredPhe,
           test="z", 
...
ContrastTitle<-paste0(unique(GroupData$FunctionGroup),collapse="_vs_")
write.csv(tidy(MGroupAndPheREML),paste0(FunctionResultsFolder,"/",ContrastTitle,"_contrast_Phe_and_group_model.csv"))
write.csv(glance(MGroupAndPheREML),paste0(FunctionResultsFolder,"/",ContrastTitle,"_contrast_Phe_and_group_model_details.csv"))

MGroupAndPheREML %>% tidy() %>% 
  kable() %>% 
  kable_styling() 
```
...
MGroupPheInter<-rma.mv(g, # for model comparison
           Vg,
           random = formula(RandomTermString), 
           mods = ~ FunctionGroup * CentredPhe,
           test="z", 
...
ContrastTitle<-paste0(unique(GroupData$FunctionGroup),collapse="_vs_")
write.csv(tidy(MGroupPheInterREML),paste0(FunctionResultsFolder,"/",ContrastTitle,"_contrast_Phe_group_interaction_model.csv"))
write.csv(glance(MGroupPheInterREML),paste0(FunctionResultsFolder,"/",ContrastTitle,"_contrast_Phe_group_interaction_details.csv"))

MGroupPheInterREML %>% tidy() %>% 
  kable() %>% 
  kable_styling() 
```
```{r}
GroupData$PredictedG<-fitted(MGroupPheInterREML)
ModelPlot <- ggplot(GroupData,aes(x=CentredPhe,y=g,shape=FunctionGroup,color=FunctionGroup)) + geom_point(size=2) 
ModelPlot <- ModelPlot + geom_line(aes(x=CentredPhe,y=PredictedG,color=FunctionGroup))
ModelPlot
```

```{r}
AIC(MNoMods,MPheOnly,MGroupOnly,MGroupAndPhe,MGroupPheInter)
```

```{r}
# compare no current Phe model to model with a main effect of Phe and function group
anova(MPheOnly,MGroupAndPhe)
```

```{r}
# compare model with no moderator to model with only function group
anova(MNoMods,MGroupOnly)
```

```{r}
# compare model with main effects to model with an 
# interaction between current Phe and function group
anova(MGroupPheInter,MGroupAndPhe)
```
...
for (CurrentGroup in unique(GroupInfoDF$FunctionGroup)){
  ...
  M1<-rma.mv(g,Vg,random = formula(RandomTermString),
             test="z", data=CurrentData, method="REML",
             control=list(optimizer="optim", optmethod="BFGS"))
  cat('<br>\n')
  M1 %>% tidy() %>% kable(caption=CurrentGroup) %>% 
    kable_styling() %>% 
    htmltools::HTML() %>%
    print
  cat("\n")
